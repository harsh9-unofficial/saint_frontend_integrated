...
const Sizes = () => {
  ...
  const fetchSizes = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${USER_BASE_URL}/sizes`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        await axios.delete(`${USER_BASE_URL}/sizes/${id}`, {
          headers: {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-[#527557] cursor-pointer text-white px-4 py-2 rounded-lg transition-sizes"
          >
